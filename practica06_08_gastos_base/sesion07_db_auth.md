...
1. Si mañana quisieras cambiar de SQLite a Postgres, ¿qué archivo específico tendrías que tocar? ¿Y qué archivos NO tendrías que tocar?
   - **Archivos que tendrías que tocar:**
     - `.env`: Cambiar `DATABASE_URL` a la cadena de conexión de PostgreSQL (ej. `postgresql+psycopg://...`).
     - `app/database.py`: Quitar o condicionar `connect_args={"check_same_thread": False}` (exclusivo de SQLite).
   - **Archivos que NO tendrías que tocar:**
     - `app/services/` y `app/routers/`: Intactos; están desacoplados del motor de base de datos gracias a la arquitectura en capas y DIP.
     - `app/models/`: Los modelos SQLAlchemy son agnósticos a la base de datos relacional.
     - `alembic/versions/`: La migración ya generada es portable y corre igual en Postgres.
...
2. ¿Qué pasaría si alguien intentara hacer `GET /gastos/` pasando el `usuario_id` de otra persona en la URL en vez de en el token? ¿Podría ver los gastos de otro usuario, con el código que escribiste hoy?
   ...
   - **Motivo:** La ruta `GET /gastos/` ni siquiera recibe `usuario_id` como parámetro de consulta ni de ruta. El `usuario_id` se extrae forzosamente de `usuario_actual.id`, el cual proviene del token JWT validado por la dependencia `get_current_user`. Esto evita vulnerabilidades de tipo IDOR / BOLA.
...
> "Si mañana un atacante consigue mi archivo `.env`, podría **firmar tokens JWT falsos haciéndose pasar por cualquier usuario y acceder a la base de datos configurada**, pero NO podría **conocer las contraseñas en texto plano de los usuarios**, porque **se almacenan únicamente como hashes unidireccionales generados con bcrypt y salt**."
...
- [x] `.env.example` existe y no contiene ningún valor real (solo la lista de variables)
- [x] `.env` generado a partir de `.env.example`, con un `SECRET_KEY` generado de forma segura (no escrito a mano)
- [x] `app/config.py` con `pydantic-settings` leyendo `.env`
- [x] `models/usuario.py` y `models/gasto.py` (con FK `usuario_id`)
- [x] Migración de Alembic aplicada (`alembic upgrade head`), con `target_metadata` apuntando a `Base.metadata` y la URL tomada de `settings`, no del placeholder de `alembic.ini`
- [x] `repositories/gastos.py` reemplazado por la versión con SQLAlchemy
- [x] `services/gastos.py` actualizado, `_validar_gasto` sin cambios respecto a S6
- [x] `RepositorioFalso` en `tests/test_gastos.py` actualizado a la nueva firma (`db`, `usuario_id`), y los 4 tests de la Sesión 6 siguen pasando
- [x] `POST /usuarios/` y `POST /usuarios/token` funcionando
- [x] `POST /gastos/` y `GET /gastos/` protegidos — `401` sin token, funcionan con token
- [x] Paginación (`skip`/`limit`) funcionando en `GET /gastos/`
- [x] Tests unitarios (S6, actualizados) + tests de integración (hoy) pasando juntos
- [x] Logs visibles en consola con nivel, ruta y status en cada request
- [x] Un error no controlado devuelve `500` con `{"detail": "Error interno del servidor"}`, nunca un stack trace
- [x] `tests/test_api_gastos.py` pasa usando `app.dependency_overrides` sobre `get_gastos_repo`, `get_db` y `get_current_user`
